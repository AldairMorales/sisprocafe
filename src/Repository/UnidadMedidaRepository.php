 ...
 */
class UnidadMedidaRepository extends ServiceEntityRepository implements BaseRepository
{
    private $security;
    ...
    {
        $queryBuilder = $this->createQueryBuilder('unidadMedida')
            ->select(['unidadMedida', 'config'])
            ->join('unidadMedida.config', 'config')
        ;

        $this->security->configQuery($queryBuilder, true);

        Paginator::queryTexts($queryBuilder, $params, ['unidadMedida.nombre']);
